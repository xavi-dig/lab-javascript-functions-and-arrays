...
function maxOfTwoNumbers(num1, num2) {
  if (num1 === num2) {
    ...
    return `In this case the bigger number corresponds to number 1 which is ${num1}`;
  } else {
    return `In this case the bigger number corresponds to number 2 which is ${num2}`;
  }
}
...
console.log(`The sum is ${sum(mixedArr)}`);
...
function averageNumbers(nArray) {
  ...
  for (let i = 0; i < nArray.length; i++) {
    finalN += nArray[i];
    finalN2 = finalN / nArray.length;
  }
  return finalN2;
}

console.log(`The result is ${averageNumbers(numbersAvg)}`);
